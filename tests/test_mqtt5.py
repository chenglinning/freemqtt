...
import gmqtt
from .utils import Callbacks, cleanup, clean_retained
...
async def test_shared_subscriptions(init_clients):
    ...
    pubclient.set_auth_credentials(username, password)
...
async def test_assigned_clientid():
    ...
    noidclient.set_auth_credentials(username, password)
    callback = Callbacks()
...
async def test_redelivery_on_reconnect(init_clients):
    ...
    disconnect_client.set_auth_credentials(username, password)
