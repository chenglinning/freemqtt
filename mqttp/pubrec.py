...
class Pubrec(Packet):
    def __init__(self, ver:int=protocol.MQTT311) -> None:
        super(Pubrec, self).__init__(ver, pktype.PUBREC)
        self.rcode = 0
        self.propset = PropertSet(pktype.PUBREC)
    ...
    def unpack(self, r: io.BytesIO) -> bool:
        if self.flags() != 0x00:
            logging.error("Error pubrec flags: %02X" % self.flags())
            return False
        ...
        if self.version == protocol.MQTT50 :
            ...
            if ReasonCode.valid4pktype(rcode, self.type):
                ...
                logging.error("Invalid pubrec packet reason code: %02X" % rcode)
                return False
